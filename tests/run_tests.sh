...
check_service_running() {
    ...
    docker service ls --filter "name=${service_name}" --format "$FORMAT_NAME" | grep -q "${service_name}"
    return $?
}
...
check_secret_exists() {
    local secret_name="$1"
    docker secret ls --filter "name=${secret_name}" --format "$FORMAT_NAME" | grep -q "${secret_name}"
    return $?
}
...
check_network_exists() {
    ...
    docker network ls --filter "name=${network_name}" --format "$FORMAT_NAME" | grep -q "${network_name}"
    return $?
}
...
check_volume_exists() {
    local volume_name="$1"
    docker volume ls --filter "name=${volume_name}" --format "$FORMAT_NAME" | grep -q "${volume_name}"
    return $?
}
