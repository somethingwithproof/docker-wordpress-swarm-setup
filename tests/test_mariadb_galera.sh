...
readonly DB_SERVICE_NAME='wordpress_wpdbcluster'
...
check_galera_running() {
    local container_id=$(get_container_id "${DB_SERVICE_NAME}")
...
check_replication() {
    ...
    local container_ids=($(docker ps --filter "name=${DB_SERVICE_NAME}" --format "{{.ID}}"))
...
if ! docker service ls --filter "name=${DB_SERVICE_NAME}" --format "{{.Name}}" | grep -q "${DB_SERVICE_NAME}"; then
...
container_id=$(get_container_id "${DB_SERVICE_NAME}")
